...
/*!
 @brief
    ...
    evtの値がEVT_BREAK_POINT、 EVT_WATCH_POINTの時はthread_id、addrに有効な値が入る。
 
    それ以外の時はthread_id、addrは不定。
 
    evtの値がEVT_BREAK_POINT、 EVT_WATCH_POINTの時、DEBUG_CB_CONTINUEを返すと処理を続行する。
...
/*!
 @brief
    ...
    イベントコールバックでDEBUG_CB_STOPを返した後に、アプリケーションの実行を再開する時に呼ぶ。
...
/*!
 @brief
    ...
    ブレークポイントがヒットするとコールバックする。
 
    ブレークポイントを無効にする場合はaddrに0を指定する。
...
/*!
 @brief
    ...
    addrは4の倍数とすること。アクセスできる領域は.textセクションの範囲内。
